...
parser.add_argument("--name", type=str, default="liconic_module", help="Module name")
    ...
    if state == ModuleStatus.BUSY:
        ...
        try:
            ...
            if action_handle == "get_current_temp":
                ...
                step_response.action_msg = str(
                    liconic.climate_controller.current_temperature
                )
            elif action_handle == "get_target_temp":
                state = ModuleStatus.IDLE
                step_response.action_response = StepStatus.SUCCEEDED
                step_response.action_msg = str(
                    liconic.climate_controller.target_temperature
                )
                ...
                step_response.action_response = StepStatus.SUCCEEDED
                step_response.action_msg = str(
                    liconic.climate_controller.current_humidity
                )
            elif action_handle == "get_target_humidity":
                state = ModuleStatus.IDLE
                step_response.action_response = StepStatus.SUCCEEDED
                step_response.action_msg = str(
                    liconic.climate_controller.target_humidity
                )
                ...
                try:
                    ...
                    if liconic.shaker_controller.shaker_is_active:
                        ...
                        liconic.shaker_controller.activate_shaker()
                    step_response.action_response = StepStatus.SUCCEEDED
                    step_response.action_msg = (
                        "Liconic shaker activated, shaker speed: "
                        + str(liconic.shaker_controller.shaker_speed)
                    )
                except ValueError:
                ...
                try:
                    ...
                    if module_resources.is_location_occupied(stacker, slot):
                        ...
                        liconic.plate_handler.move_plate_from_transfer_station_to_slot(
                            stacker, slot
                        )
                        time.sleep(20)
                        ...
                        step_response.action_response = StepStatus.SUCCEEDED
                        step_response.action_msg = (
                            "Plate loaded into liconic stack "
                            + str(stacker)
                            + ", slot "
                            + str(slot)
                        )
                ...
                try:
                    ...
                    if module_resources.is_location_occupied(stacker, slot):
                        ...
                        step_response.action_response = StepStatus.SUCCEEDED
                        step_response.action_msg = (
                            "Plate unloaded from liconic stack "
                            + str(stacker)
                            + ", slot "
                            + str(slot)
                        )
